...
EOT;
    ...
    public function menu(): string
    {
        ...
        foreach ($this->get('config', 'menuItems') as $item) {
            ...
            $output .= '<li' . ($this->currentPage === $item->slug ? ' class="active"' : ' class="nav-item"') . '><a class="nav-link" href="' . self::url($item->slug) . '">' . $item->name . '</a></li>';
        }
    ...
    public function settings(): string
    {
        ...
        $output = '<div id="save"><h2>Saving...</h2></div><div id="adminPanel" class="container-fluid"><div class="text-right padding20"><a data-toggle="modal" class="padding20" href="#settingsModal"><b>Settings</b></a><a href="' . self::url('logout&token=' . $this->getToken()) . '">Logout</a></div><div class="modal" id="settingsModal"><div class="modal-dialog modal-xl"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button></div><div class="modal-body col-xs-12"><ul class="nav nav-tabs text-center" role="tablist"><li class="nav-item" role="presentation" class="active"><a class="nav-link"  href="#currentPage" aria-controls="currentPage" role="tab" data-toggle="tab">Current page</a></li><li class="nav-item" role="presentation"><a class="nav-link" href="#general" aria-controls="general" role="tab" data-toggle="tab">General</a></li><li class="nav-item" role="presentation"><a class="nav-link" href="#files" aria-controls="files" role="tab" data-toggle="tab">Files</a></li><li class="nav-item" role="presentation"><a class="nav-link" href="#themesAndPlugins" aria-controls="themesAndPlugins" role="tab" data-toggle="tab">Themes & plugins</a></li><li class="nav-item" role="presentation"><a class="nav-link" href="#security" aria-controls="security" role="tab" data-toggle="tab">Security</a></li></ul><div class="tab-content col-md-8 col-md-offset-2"><div role="tabpanel" class="tab-pane active" id="currentPage">';
        if ($this->currentPageExists) {
        ...
        foreach ($fileList as $file) {
            $output .= '<a href="' . self::url('?deleteFile=' . $file . '&token=' . $this->getToken()) . '" class="btn btn-xs btn-danger" onclick="return confirm(\'Delete ' . $file . '?\')" title="Delete file">&times;</a><span class="marginLeft5"><a href="' . self::url('data/files/') . $file . '" class="normalFont" target="_blank">' . self::url('data/files/') . '<b class="fontSize21">' . $file . '</b></a></span><p></p>';
        }
